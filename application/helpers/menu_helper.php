...
function app_init_admin_sidebar_menu_items()
{
    ...
    $CI->app_menu->add_sidebar_menu_item('marketplace', [
         'collapse' => true,
         // 'slug'    => 'marketplace',
    ...
    if (has_permission('marketplace', '', 'view') && has_permission('marketplace', '', 'edit')) {
       $CI->app_menu->add_sidebar_children_item('marketplace', [
                'slug'     => 'view_overview',
                'name'     => _l('als_view_overview'),
                'href'     => admin_url('marketplace'),
                'position' => 5,
        ]);
       $CI->app_menu->add_sidebar_children_item('marketplace', [
                'slug'     => 'edit_my_profile',
                'name'     => _l('als_edit_my_showroom_profile'),
                'href'     => admin_url('marketplace/edit'),
                'position' => 10,
        ]);
    }
    if (has_permission('marketplace', '', 'view') && !has_permission('marketplace', '', 'edit')) {
    ...
    if (has_permission('subscriptions', '', 'view') || has_permission('subscriptions', '', 'view_own')) {
        $CI->app_menu->add_sidebar_menu_item('subscriptions', [
         ...
         // if (has_permission('subscriptions', '', 'view') || has_permission('subscriptions', '', 'view_own')) {
        $CI->app_menu->add_sidebar_children_item('subscriptions', [
                'slug'     => 'blocks',
                'name'     => _l('als_blocks'),
                'href'     => admin_url('subscriptions'),
                'position' => 5,
       ...
       $CI->app_menu->add_sidebar_children_item('subscriptions', [
                'slug'     => 'settings',
                'name'     => _l('als_sub_settings'),
                'href'     => admin_url('subscriptions/settings'),
                'position' => 10,
        ]);
    // }
    }
