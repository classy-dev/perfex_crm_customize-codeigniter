    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            if ($field['type'] == 'input' || $field['type'] == 'number') {
                ...
                $id_str = str_replace(['[', ']'], '_', $select_name);
                ...
                $fields_html .= '<select ' . $select_attrs . ' name="' . $select_name . '" id="'.$id_str.'" class="' . ($items_add_edit_preview == false ? 'select-placeholder ': '') . 'selectpicker form-control' . ($field['type'] == 'multiselect' ? ' custom-field-multi-select' : '') . '" data-width="100%" data-none-selected-text="' . _l('dropdown_non_selected_tex') . '"  data-live-search="true">';
