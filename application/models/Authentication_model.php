...
class Authentication_model extends App_Model
{
    ...
    public function get_created_date_db($email){
        $query = $this->db->query("select datecreated from tblstaff where email='$email'");
            return $query->result_array();
    }

    public function get_role_status_db($email){
        $query = $this->db->query("select admin, pay_status from tblstaff where email='$email'");
            return $query->result_array();
    }

    public function get_role($id){
        $query = $this->db->query("select role from tblstaff where staffid='$id'");
            return $query;
    }

    public function get_name($email){
        $query = $this->db->query("select firstname, lastname from tblstaff where email='$email'");
            return $query->result_array();
    }
    ...
    public function set_transaction($data){
        $this->db->insert('tbltrial_payment',$data);
    }

    public function get_staffid($email){
       $query = $this->db->query("select staffid from tblstaff where email='$email'");
            return $query->result_array(); 
    }
